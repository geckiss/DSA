...
namespace structures
{
	...
	{
		TreapItem<K, T>* newItem = new TreapItem<K, T>(key, data, (*generator_)());
	...
	{
		bool found = false;
		typename BSTTreeNode* node = findBSTNode(key, found);
		if (found) {
			T data = node->accessData();
			node->minimizePriority();
			while (!isHeapOk()) {
				if (node->isLeftSon()) {
	...
	{
		return (!node->hasParent() || extractPriority(node) > extractPriority(node->getParent())) &&
			(!node->hasLeftSon() || extractPriority(node) < extractPriority(node->getLeftSon())) &&
			(!node->hasRightSon() || extractPriority(node) < extractPriority(node->getRightSon()));
	}
	...
	{
		if (node->isRightSon()) {
			BSTTreeNode* left = node->changeLeftSon(nullptr);
	...
	{
		if (node->isLeftSon()) {
			BSTTreeNode* right = node->changeRightSon(nullptr);
			BSTTreeNode* parent = node->getParent();
			BSTTreeNode* grandParent = parent->getParent();
