...
namespace structures 
{
	...
	public:
		...
		/// <summary> Vlozi data s danym klucom do tabulky BEZ KONTROLY UNIKATNOSTI KLUCA. </summary>
		/// <param name = "key"> Kluc vkladanych dat. </param>
		/// <param name = "data"> Vkladane data. </param>
		virtual void insertWithoutCheck(const K & key, const T & data) = 0;
	...
	{
		// Ak necheknem identitu, mozem zmazat seba, a potom nemam z coho kopirovat
		if (this != &other) {
			clear();
	...
	{
		TableItem<K, T>* it = findTableItem(key);
		if (it != nullptr) {
			return it->accessData();
		}
		throw std::exception("Cannot find item with specified key.");
	}
	...
	inline const T SequenceTable<K, T>::operator[](const K key) const
	{
		...
		return false;
		throw std::exception("Cannot find item with key.");
	}
	...
	inline void SequenceTable<K, T>::insert(const K & key, const T & data)
	{
		if (!containsKey(key)) {
			...
			throw std::exception("Cannot insert, key already in.");
		}
	}
	...
	{
		TableItem<K, T>* rem = findTableItem(key);
		if (rem != nullptr) {
			list_->tryRemove(rem);
			T data = rem->accessData();
			delete rem;
			return data;
		}
		else {
			throw std::exception("Cannot remove, item with key not found.");
		}
	}
