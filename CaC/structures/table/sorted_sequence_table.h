...
namespace structures
{
	...
	/// <param name = "K"> Kluc prvkov v tabulke. </typepram>
	/// <param name = "T"> Typ dat ukladanych v tabulke. </typepram>
	...
	public:
		...
		void insertWithoutCheck(const K& key, const T& data) override;
	protected:
	...
	inline void SortedSequenceTable<K, T>::insert(const K & key, const T & data)
	{
		if (isEmpty()) {
			...
			if (!found) {
				...
				throw std::exception("Item with key already inside.");
			}
		}
	}
	...
	inline int SortedSequenceTable<K, T>::indexOfKey(const K & key, int indexStart, int indexEnd, bool & found) const
	{
		...
		if (it != nullptr) {
			...
			if (it->getKey() < key) {
				index = indexOfKey(key, stred + 1, indexEnd, found);
			}
			else {
				index = indexOfKey(key, indexStart, stred, found);
			}
		}
		else {
			found = false;
	...
	template<typename K, typename T>
	inline void SortedSequenceTable<K, T>::insertWithoutCheck(const K & key, const T & data)
	{
	}
}