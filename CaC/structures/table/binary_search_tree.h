...
namespace structures
{
	...
	inline BinarySearchTree<K, T>& BinarySearchTree<K, T>::operator=(const BinarySearchTree<K, T>& other)
	{
		if (this != &other) {
			clear();
			Tree<TableItem<K, T>*>::PreOrderTreeIterator iterBegin(other.getRoot());
	...
	inline T & BinarySearchTree<K, T>::operator[](const K key)
	{
		...
		if (found) {
			return node->accessData();
		}
		else {
			throw std::exception("Item with specified key is not in BST.");
		}
	}
	...
	inline void BinarySearchTree<K, T>::insert(const K & key, const T & data)
	{
		...
		typename BSTTreeNode* newNode = new BSTTreeNode(newItem);
		if (tryToInsertNode(newNode)) {
			...
			delete newItem;
			delete newNode;
			throw std::exception("Cannot insert node");
		}
	...
	inline T BinarySearchTree<K, T>::remove(const K & key)
	{
		...
		if (found) {
			...
			throw std::exception("Cannot remove; item with key is not in");
		}
	}
	...
	inline bool BinarySearchTree<K, T>::tryFind(const K & key, T & data)
	{
		...
		bool found = false;
		data = (findBSTNode(key, found))->accessData();
		return found;
	}
	...
	inline typename BinarySearchTree<K, T>::BSTTreeNode* BinarySearchTree<K, T>::findBSTNode(const K & key, bool & found) const
	{
		if (isEmpty()) {
			...
			BSTTreeNode* node = binaryTree_->getRoot();
			while (key != node->accessData()->getKey()) {
	...
	inline void BinarySearchTree<K, T>::extractNode(BSTTreeNode* node)
	{
		...
		case 1:
			...
			replaceNode->getParent(node->getParent());
			break;
		case 2:
			...
			// Otec replaceNode bude mat noveho laveho syna
			if (replaceNode->hasRightSon()) {
				replaceNode->getParent()->setLeftSon(replaceNode->getRightSon());
			}
			// Nema synov
			else {
				replaceNode->getParent()->setLeftSon(nullptr);
			}

			if (node->getRightSon() == replaceNode->getParent()) {
				replaceNode->getParent()->setParent(replaceNode);
			}

			// ReplaceNode bude mat noveho praveho syna
			replaceNode->setRightSon(node->getRightSon());

			if (parent == nullptr) {
				binaryTree_->replaceRoot(replaceNode);
			}
			else {
				// Otec Node-y bude mat noveho praveho syna
				if (node->isLeftSon()) {
			...
			// ReplaceNode bude mat noveho otca(otca Node-y)
