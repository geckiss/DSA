...
namespace structures
{
	...
	inline ArrayList<T>::ArrayList() :
		// List nemusim pisat, je to implicitny konstruktor, vola sa vzdy
		//List(),
		array_(new Array<T>(20)),
		size_(0)
	{
	}
	...
	inline void ArrayList<T>::add(const T & data)
	{
		...
		// Pritupim do Array, cez operator[] pristupim do Vector, tam ziskam bytePointer pretypovany na T a 
		// dereferencujem ho tam, ziskam nove prazdne miesto v poli, priradim tam data
		(*array_)[size_++] = data;
	}
	...
	{
		DSRoutines::rangeCheckExcept(index, size_ + 1, "");
		if (index == size_) {
			add(data);
		}
		else {
			// Ci sa nemam zvacsovati
			if (size_ == array_->size()) {
				enlarge();
			}
	...
	{
		Array<T>* nove = new Array<T>(size_ + 30);
	...
	{
		ArrayListIterator ali = dynamic_cast<const ArrayListIterator&>(other);
		position_ = ali.position_;
		arrayList_ = ali.arrayList_;
		return *this;
	}
	...
	{
		ArrayListIterator ali = dynamic_cast<const ArrayListIterator&>(other);
		return (arrayList_ != ali.arrayList_ || position_ != ali.position_);
	}
