...
namespace structures
{
	...
	{
		BinaryTreeNode<T>* nahradeny = getRightSon();
	...
	{
		return getParent()->getSon(LEFT_SON) == this;
	}

	template<typename T>
	inline bool BinaryTreeNode<T>::isRightSon() const
	{
		return getParent()->getSon(RIGHT_SON) == this;
	}

	template<typename T>
	inline bool BinaryTreeNode<T>::hasLeftSon() const
	{
		return getSon(LEFT_SON) != nullptr;
	}

	template<typename T>
	inline bool BinaryTreeNode<T>::hasRightSon() const
	{
		return getSon(RIGHT_SON) != nullptr;
	}
	...
	{
		BinaryTreeNode<T>* ls = current->getSon(LEFT_SON);
		if (ls != nullptr) {
			populatePath(ls);
		}
		path_->push(current);
		BinaryTreeNode<T>* rs = current->getSon(RIGHT_SON);
		if (rs != nullptr) {
			populatePath(rs);
		}
	}
