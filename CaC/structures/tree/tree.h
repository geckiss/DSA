...
namespace structures
{
	...
	inline TreeNode<T>* TreeNode<T>::getBrother(int brothersOrder)
	{
		if (getParent() != nullptr) {
			...
			throw std::exception("TreeNode<T>::getBrother: I am (G)root.");
		}
	}
	...
	{
		// PreOrder(size = 1) alebo PostOrder(size = 0), rovnake
		int size = 1;
		for (int i = 0; i < degree(); i++) {
			TreeNode<T>* son = getSon(i);
			if (son != nullptr) {
	...
	{
		TreeIterator otherALI = dynamic_cast<const TreeIterator&>(other);
		path_ = otherALI.path_;
		return *this;
	}
	...
	{
		const TreeIterator& pom = dynamic_cast<const TreeIterator&>(other);
		return path_ != pom.path_;
	}
	...
	inline void Tree<T>::PostOrderTreeIterator::populatePath(TreeNode<T>* const current)
	{
		if (current != nullptr)
		{
			for (int i = 0; i < current->deegree(); i++)
			{
				populatePath(current->getSon(i));
			}
			path_->push(current);
