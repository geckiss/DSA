...
namespace structures
{
	...
	inline PriorityQueueList<T>& PriorityQueueList<T>::operator=(const PriorityQueueList<T>& other)
	{
		if (this != &other) {
			...
			for (PriorityQueueItem<T>* pqi : *other.list_) {
				list_->add(new PriorityQueueItem<T>(*pqi));		// nekopirujem smernik, ale to na co to ukazuje, y toho vytvaram novy smernik
			}
		}
		return *this;
	}
	...
	{
		// delete itemy na ktore ukazuju smerniky listu
		// delete smerniky
		// delete list
	...
	inline int PriorityQueueList<T>::indexOfPeek() const
	{
		...
		int index = 0;
		for (PriorityQueueItem<T>* pqi : *list_) {
			if (pqi->getPriority() < bestPriority) {
	...
	{
		int index = indexOfPeek();
		return (*list_)[index]->getPriority();
	}
}