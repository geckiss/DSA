...
namespace structures
{
	...
	public:
		...
		int numberOfSons();
	protected:
		/// <summary> Synovia vrchola. </summary>
	...
	{
		for (int i = 0; i < K; i++) {
			if (getSon(i) != nullptr) return false;
		}
		return true;
	}
	...
	{
		if (getSon(order) == nullptr) {
			(*children_)[order] = dynamic_cast<KWayTreeNode<T, K>*>(son);
		}
		else {
			throw std::exception("Son already exists; can't insert son");
		}
	}
	...
	inline TreeNode<T>* KWayTreeNode<T, K>::replaceSon(TreeNode<T>* son, int order)
	{
		...
		//(*children_)[order] = dynamic_cast<KWayTreeNode<T, K>*>(son);
		(*children_)[order] = dynamic_cast<KWayTreeNode<T, K>*>(son->accessData());
		return old;
	}
	...
	{
		TreeNode<T>* son = (*children_)[order];
		(*children_)[order] = nullptr;
		return son;
	}
