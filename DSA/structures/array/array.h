...
namespace structures 
{
	...
	//Vytvorenie sameho seba pomocou copy konstruktora
	template<typename T>
	...
	// Chcem, aby sa jedno pole rovnalo druhemu
	template<typename T>
	Array<T>& Array<T>::operator=(const Array<T>& other)
	{
		// Nepriradujem sam do seba
		if (this != &other) {
			// Pole ma stanoveny pocet prvkov, nemozem mensie dat do vacsieho a opacne
			if (size_ == other.size_)
				// Chcem priradit ten objekt, na ktory tie smerniky ukazuju(jeden objekt typu Vector bude druhy)
				// S povodnym objektom nechcem nic robit, preto musim prepisat stary objekt, na ktory ukazuje moj vector_
				// Tak ako s int i = 2, j = 3, j = i, j-- ... i ostava 2, j = 1 ... aj ked som i-cko priradil j-cku, s i-ckom som nic nerobil
	...
	// Nemozem pouzit operator[] z Vectora, je tam static_cast, potrebujem reinterpret
	template<typename T>
	...
	// POLIA!!!! Rovnaka velkost a data(hodnoty vo Vectore)
	template<typename T>
