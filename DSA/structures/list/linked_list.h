...
namespace structures
{
	...
	public:
		...
		/// <param name = "other"> Prvok jednostranne zretazeneho zoznamu, z ktoreho sa prevezmu vlastnosti.. </param>
	...
	inline LinkedList<T>::LinkedList(const LinkedList<T>& other) :
		LinkedList()
	{
		// Seba dereferujem a nad sebou zavolam =
		*this = other;
	}
	...
	{
		LinkedListItem<T>* lli = new LinkedListItem<T>(data);
		if (first_ == nullptr) {
			first_ = lli;
		}
		else {
			// Tomu objektu, na ktory smeruje last, nastavim pointer na dalsi na lli
			last_->setNext(lli);
		}
		last_ = lli;
		size_++;
	...
	inline void LinkedList<T>::insert(const T & data, const int index)
	{
		...
		if (index == size_) {
			...
			if (index == 0) {
				...
				LinkedListItem<T>* pred = getItemAtIndex(index);
				item->setNext(pred->getNext());
				pred->setNext(item);
			}
			size_++;
		}
	...
	{
		DSRoutines::rangeCheckExcept(index, size_ - 1, "Index out of bounds");
		T data;
		LinkedListItem<T>* mazany;
		LinkedListItem<T>* predMazanym;
		if (index == 0 && first_ != nullptr) {
			mazany = first_;
			...
			if (index == 1) {
				...
				predMazanym = getItemAtIndex(index - 1);
			}
			mazany = predMazanym->getNext();
			predMazanym->setNext(mazany->getNext());
		}
		data = mazany->accessData();
		delete mazany;
		size_--;
		if (size_ == 0) last_ = nullptr;
		if (size_ == index) last_ = predMazanym;
		return data;
	}
	...
	{
		LinkedListItem<T>* lli = first_;
		for (int i = 0; i < size_; i++) {
			if (lli->accessData() == data) {
				return i;
			}
			lli = lli->getNext();
		}
	...
	{
		if (size_ != 0) {
			LinkedListItem<T>* mazany_ = first_;
			LinkedListItem<T>* dalsi_ = first_->getNext();

			do {
				delete mazany_;
				mazany_ = dalsi_;
				if (mazany_ != nullptr) dalsi_ = mazany_->getNext();
			} while (dalsi_ != nullptr);
	...
	{
		DSRoutines::rangeCheckExcept(0, size_ + 1, "Invalid index");
		LinkedListItem<T>* lli = first_;
	...
	{
		LinkedListIterator lli = dynamic_cast<const LinkedListIterator&>(other);
		position_ = lli.position_;
		return *this;
	}
	...
	{
		LinkedListIterator lli = dynamic_cast<const LinkedListIterator&>(other);
		return (this->position_ != lli.position_);
	}
